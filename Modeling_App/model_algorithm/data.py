...
def fetch_train_test_split(input_folder: os.PathLike, random_seed: int = 123456, n_splits: int = 10) -> TrainTestSet:
    ...
    data = data = pd.DataFrame(data= np.c_[iris['data'], iris['target']],
                     columns= ['sepal_width', 'selpal_height', 'petal_width', 'petal_height', 'target'] 
                    )
    ...
    X, Y = data.loc[:, features[:-1]], np.log(data.loc[:, 'target'])
