...
The goal is to move to the green square (for +1 reward) and avoid the red
square (for -1 reward). The position of the three blocks is randomized
every episode.'''
...
class Qnetwork():
    def __init__(self, h_size):
        ...
        self.Q = tf.reduce_sum(tf.multiply(self.Qout,
...
# Experience Replay
# This class allows us to store experies and
#  sample then randomly to train the network.
...
# These functions allow us to update the parameters
# of our target network with those of the primary network.
...
# The size of the final convolutional layer before splitting it into
...
with tf.Session() as sess:
    ...
    for i in range(num_episodes):
        ...
        j = 0
        # The Q-Network
        # If the agent takes longer than 200 moves to reach
        #  either of the blocks, end the trial.
        while j < max_epLength:
            j += 1
            # Choose an action by greedily (with e chance of
            # random action) from the Q-network
