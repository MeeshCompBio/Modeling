...
# Model Network
# Here we implement a multi-layer neural network that predicts the next
...
true_observation = tf.placeholder(tf.float32, [None, 4],
                                  name="true_observation"
                                  )
...
done_loss = (tf.multiply(predicted_done, true_done) +
             tf.multiply(1-predicted_done, 1-true_done)
             )
...
# This function uses our model to produce a new state when
    ...
    while episode_number <= 5000:
        ...
        if done:
            ...
            if switch_point + batch_size == episode_number:
                ...
                if not drawFromModel:
                    print('World Perf: Episode %f. Reward %f. action: %f. \
                        mean reward %f.' %
                          (real_episodes,
                           reward_sum/real_bs,
                           action,
                ...
                # from the model and training the model from
